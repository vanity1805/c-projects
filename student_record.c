    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 3:
                ...
                break;
            case 4:
                printAllStudentRecords(file); // New case
                break;
            case 5:
                fclose(file);
                return 0;
...
void printStudentsWithSameBirthMonth(FILE *file, int month) {
    ...
    while (fread(&student, sizeof(Student), 1, file)) {
        if (student.birthMonth == month) {
            printf("Name: %s %s, Age: %d, Sex: %s, Birthday: %02d-%02d-%04d\n",
                   student.fname, student.lname, student.age, student.sex, student.birthDay, student.birthMonth, student.birthYear);
        }
    }
}

void printAllStudentRecords(FILE *file) {
    Student student;

    rewind(file); // Move the file pointer to the beginning before reading
...
int compareNames(const void *a, const void *b) {
    ...
    int result = strcmp(studentA->fname, studentB->fname);
    if (result == 0) {
